...
> what remains.
>
> Deployed to `mcapp.local` as `v1.6.14-dev.47` and verified against live traffic. That deploy
> also **settled one of the survey's UNKNOWNs**: the negotiated BLE ATT MTU is **255 bytes**, and
> BlueZ does expose it on this GATT path. That turns the RfC's proposal to append a 96-character
> filter list to the `I` register from a vague risk into arithmetic against a hard ceiling — see
> B29, 15.10 and 16.1 item 6.
...
| #   | Case                                                 | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| --- | ---------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| B29 | BLE MTU on the inbound path                          | **Measured 2026-08-20 on `mcapp.local`: the negotiated ATT MTU is 255 bytes.** This was UNKNOWN when the survey was written and is now a hard number. Only **outbound** writes were length-checked in `ble_service`; inbound notifications were checked by nothing, and **there is still no multi-notification reassembly anywhere** — adding it is a protocol change needing firmware agreement. So a register JSON that exceeds one notification is truncated at the GATT layer, `json.loads` raises, and **the entire register update is discarded** — callsign, firmware version and group config with it — not merely the field that overflowed. `1f5f2ee` makes that loud (truncation logged at ERROR, distinguished from genuine malformation) and logs the negotiated MTU once on connect, but does not and cannot prevent it. |
...
| P5  | medium   | **No inbound BLE MTU guard.** Only outbound writes are length-checked; a truncated inbound `I` register drops the whole register update silently. See B29.                                                                                                                                                                                                                                                                                                                                                                                                              | `MCProxy/ble_service/src/ble_adapter.py:87` vs `:1229`                                    | **fixed** `1f5f2ee` — made loud; MTU measured at 255 B; reassembly still absent by design |
...
- **The filter list has to fit through BLE, and the RfC never says so.** Section 7.7 costs the
  96-char buffer in NVS and in RAM (`char[96]` on every board). It does not mention the transport.
  The phone apps and MCProxy both learn a node's configuration from the `I` register over BLE, and
  that register has to arrive in **one** GATT notification: measured 2026-08-20 on `mcapp.local`,
  the negotiated ATT MTU is **255 bytes**. Nothing on our side reassembles a split frame, and the
  failure is not graceful degradation — the whole register update is dropped, so callsign, firmware
  version and group config all silently stop refreshing along with the filter list. A 96-character
  addition to a register with an unknown current size, against a 255-byte ceiling, is a real
  possibility of exactly that. Costing the buffer in flash and RAM but not on the wire it travels
  over is the gap.
...
6. **The `I` register has a hard 255-byte ceiling, and the RfC's filter list eats into it.** Measured
   on `mcapp.local` 2026-08-20: the negotiated ATT MTU is **255 bytes**, and BlueZ does populate
   `GattCharacteristic1.MTU` on this GATT path (the survey could not determine that; it is now
   settled). There is no inbound reassembly on our side and none planned, so a register JSON above
   that ceiling is truncated at the GATT layer and **the whole update is lost, not just the

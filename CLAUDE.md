...
- **A subscribe POST replaces the stored filter wholesale, and that is load-bearing in both directions.** It is the only prefs channel the contract has, so the backend cannot distinguish "the user cleared their groups" from "the client POSTed before its own settings finished loading" — which is exactly how the webapp silently wiped a live subscription's groups on 2026-08-17 (fixed client-side in webapp v1.6.14-dev.42, see its `docs/backlog.md` B2). **Do not add a server-side heuristic** that ignores a default-looking filter or merges it into the stored one: that would break clearing groups on purpose, and it would diverge from mc-chat. The fix belongs in every client — never POST a filter your store has not supplied.
